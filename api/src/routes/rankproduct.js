...
router.post("/", async (req, res) => {
  let { customer_id, product_id, rank, comment } = req.body;

  try {
    return res
      .status(200)
      .json(
        await RoutFunc.createNewRank(customer_id, product_id, rank, comment)
      );
  } catch (error) {
...
router.put("/", async (req, res) => {
  let { id, rank, comment } = req.body;

  try {
    return res.status(200).json(await RoutFunc.ModifyRank(id, rank, comment));
  } catch (error) {
...
router.delete("/:id", async (req, res) => {
  let { id } = req.params;
  try {
    return res.status(200).json(await RoutFunc.DeleteRank(parseInt(id)));
  } catch (error) {
